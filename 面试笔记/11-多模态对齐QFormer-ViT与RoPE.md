# 11｜多模态对齐：Q-Former、ViT 与 2D RoPE

**本篇专讲多模态接口**。**1D RoPE** 见 **`02` §7**；**长窗外推** 见 **`07` §1～§3**；PPL/mask 手撕见 [`手撕代码/AI手撕/03-手撕PPL.md`](../手撕代码/AI手撕/03-手撕PPL.md)。
...
文本 token 天然是 **一维序列**（第 1 个词、第 2 个词…），**1D RoPE** 给每个位置一个 **沿序列的相位**（详见 **`02` §7**）。  
...
「文本 1D RoPE 见 02；图像 2D RoPE 或展平近似；点积只依赖相对距离。」
